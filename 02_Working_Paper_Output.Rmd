...
toc()
## Can run this instead of the chunk below. Numbering is also now correct, unless added later with find_add_models
new_ls_theory <- find_add_models(ls_theory = ls_theory, on_ls=TRUE, add_nodes = list(var_names = "f", timing = 1, types = "ctr", node_names = "X4"), user_mods = c("a ~ b + d; b ~ c + d; f ~ a + b"))
```
