...
#### Identify Compatible (testing)


```{r identify_compatible}
comp_matrix <- identify_compatible(full_matrix, ref_mod=1)
full_matrix_cmp <- left_join(full_matrix, comp_matrix, by="model")
...
* Here is a BUG
I identified X1 as taking place at -2 and X2/X3 at -3, but the generation of 

```{r explot}
dag_plots2 <- list()

for (l in 1:length(full_matrix_dags2)){
    plot(full_matrix_dags2[[l]], xlim = c(-2.1,1.1), ylim = c(-1.1,1.1))
    text(0.5,-1, paste0("MAS = ",full_matrix2$mas[l]))
    text(0,1, paste0("Model ",full_matrix2$model[l]), font = 2)
    dag_plots2 <- append(dag_plots2, list(recordPlot()))
}
```
