---
title: "theoRy: Deprecated Pure-R Workflow"
output:
  pdf_document:
    fig_width: 4.5
    fig_height: 3
...
**Authors**: Hung H. V. Nguyen and Nate Breznau.
...
> **Legacy vignette.** The functions demonstrated here are the deprecated,
> pure-R workflow retained for historical analyses. The current package uses an
> R client with a Python FastAPI backend; see
> `02_Pipeline_and_Visualization.Rmd` for `start_theory_engine()`,
> `build_component_registry()`, `expand_model_states()`,
> `build_dyad_matrix()`, `compute_delta_u()`, and `run_simulation()`.
>
> The modern API exposes exactly `similarity_rate`, `mas_compatible`, and
> `identified_compatible`. Its causal metrics use explicit exposure/outcome
> queries and Dagitty adjustment sets; `identified_compatible` uses the native
> fixed-direct complete-conditioning predicate rather than general ID through
> R `causaleffect`.
...
```
\newpage
## Legacy: Build Causal Matrix (`build_causal_node`)
...
\newpage
## Legacy: Build Formula Matrix (`build_formula_matrix`)
...
The deprecated `add_compatible()` function compares a reference model with the
legacy pure-R universe and emits `test_compatible` and
`full_model_compatible`. The latter is a distinct historical concept: it is
not a modern compatibility metric, is not general causal identification, and
must not be interpreted as `identified_compatible`.
...
\newpage
## Other Legacy Utility Functions

Other deprecated utility functions add or find models (`find_add_models()`) or
build a set matrix for QCA (`build_set_matrix()`). They do not call the Python
engine and are separate from the current registry/state/dyad workflow.
