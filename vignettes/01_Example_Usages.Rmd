...
user_mods <- c("Y ~ Xtest + X2; Xtest ~ X1 + X2", # Model 1
                ...
                "Y ~ Xtest + X2; Xtest ~ X2; X3 ~ X3" # Model 5
                )
...
# Add models 6 and 7
test <- find_add_models(ls_theory = multiverse_2, on_ls = TRUE, 
                user_mods = c("Y ~ Xtest + X2 + X3; Xtest ~ X1 + X2; X3 ~ X2",
                              "Y ~ Xtest + X2; Xtest ~ X1 + X2; X3 ~ X3"), 
                assert_mod_num = c(6,7))


causal_matrix_merge <- rbind(
    as.data.frame(multiverse_1$causal_matrix) %>%
        mutate(multiverse = 1,
               model=ifelse(model %in% c(6,7), model+length(unique(multiverse_1$causal_matrix$model)), model)),
    as.data.frame(multiverse_2$causal_matrix) %>%
        mutate(multiverse = 2,
               model =ifelse(model %in% c(6,7), model,model+length(unique(multiverse_1$causal_matrix$model)))) %>%
        select(-prev_mod)
)
